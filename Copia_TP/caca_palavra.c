...
void impremematriz (char **tabu,int lin,int col){
  ...
  for (i = 0; i < col;i++)//montando o tabuleiro
    ...
    for (i = 0;i < lin;i++)
    {
      ...
      {
        if (isupper(tabu[i][j])){
          printf(BOLD(GREEN(" %c "))TAB_VER,tabu[i][j]);
        }
        else{
...
int main (){
  ...
  case 1:
    ...
    if (dificuldade == 1)//apenas palavras na vertical e horizontal
    {
      tabu = facil(tabu,matriz,numero_palavras,lin,col,loca);
    }
    else if (dificuldade == 2)//palavras na horizontal, vertical e diagonal
    {
      tabu = medio(tabu,matriz,numero_palavras,lin,col,loca);
    }
    else if (dificuldade == 3)//horizontal,vertical e diagonal(ao contrario tambem)
    {
